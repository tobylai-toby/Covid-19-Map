...
def chinaCovid19Map(type_=None, size=None) -> Union[dict, Map]:
    if type_ is None:
        type_ = ["confirmed"]
    if size is None:
        size = ["900px", "500px"]
    ...
    cn_map = (
        ...
        .set_global_opts(title_opts=opts.TitleOpts(subtitle=f"累计确诊/无症状：{_ill}"), visualmap_opts=opts.VisualMapOpts(max_=max_aver))
    )
...
def chinaCovid19MapRelative(type_ = None, size = None) -> Union[dict, Map]:
    if type_ is None:
        type_ = ["confirmed"]
    if size is None:
        size = ["900px", "500px"]
    ...
    provinces = [cdata["area"] for cdata in chinaData]  # 省份
    confirmed = [int(cdata["confirmedRelative"])
                 for cdata in chinaData]  # 累计确诊
    died = [int(cdata["diedRelative"]) for cdata in chinaData]  # 累计死亡
    crued = [int(cdata["curedRelative"]) for cdata in chinaData]  # 累计治愈
    asymptomatic = [int(cdata["asymptomaticRelative"] or "0")
                    for cdata in chinaData]  # 累计无症状
